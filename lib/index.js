    ...
    visitor: {
      ...
      ExpressionStatement(path, state) {
        ...
        function transpileASSERT() {
          ...
          const nexpr = t.callExpression(t.clone(callee.node), [calleeLoc()].concat(assertions.map(a => t.arrayExpression([a.caption, a.expr, t.objectExpression(a.aditionalFields)]))));
          path.get('expression').replaceWith(nexpr);
          ...
          function add(caption, expr, aditionalFields) {
            const a = {
              caption,
              expr,
              aditionalFields
          ...
          function explainExpr(bigExpr, aditionalFields, addf) {
            if (t.isStringLiteral(bigExpr)) {
              ...
              if (addf) aditionalFields.push(t.objectProperty(t.stringLiteral(bigExpr.name), t.clone(bigExpr)));
              return {
              ...
              bigExpr.arguments.forEach(a => {
                explainExpr(a, aditionalFields, true);
              });
              ...
              const left = explainExpr(bigExpr.left, aditionalFields, true);
              const right = explainExpr(bigExpr.right, aditionalFields, true);
              ...
              aditionalFields.push(t.objectProperty(t.stringLiteral(caption), t.clone(valId)));
              return {
                caption,
                val: t.clone(valId)
              };
            } else if (t.isUnaryExpression(bigExpr)) {
              const arg = explainExpr(bigExpr.argument, aditionalFields, true);
              const caption = (0, _generator.default)(bigExpr).code;
