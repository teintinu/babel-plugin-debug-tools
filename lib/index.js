  ...
  const visitor = {
    ...
    visitor: {
      ...
      ExpressionStatement(path, state) {
        ...
        if (isH5) {
          if (mode === 'production') {
            ...
            if (property.isIdentifier({
              ...
              name: 'TRACE'
            })) transpileLOGorTRACE(); else if (property.isIdentifier({
              name: 'CHECK'
            })) {
              transpileOthers();
            } else if (property.isIdentifier({
        ...
        function transpileTRACE() {
          transpileLOGorTRACE();
        }

        function transpileLOGorTRACE() {
            ...
            if (t.isStringLiteral(cclone)) prev.push(cclone); else if (t.isObjectExpression(cclone)) {
              cclone.properties.forEach(p => {
                if (t.isObjectProperty(p)) {
                  prev.push(t.arrayExpression([p.key, p.value]));
                }
              });
            } else {
        ...
        function transpileOthers() {
          path.get('expression').replaceWith(t.logicalExpression('&&', t.identifier(identifier), t.clone(expr.node)));
        }
